...
use crate::{provider::Error as ProviderError, Result};
...
pub trait ThreadedChannelRequestor {
    ...
    /// Caches an out-of-order response or currently unneeded response. Realistically, this is just
    ...
    fn get_and_handle_response<T>(
        ...
        match_func: &dyn Fn(
            &mut Self,
            DatabaseResponse<MAX_PREFETCH>,
            DatabaseRequest<MAX_PREFETCH>,
        ) -> Result<Option<T>>,
    ) -> Result<Option<T>> {
...
pub trait ThreadedChannelAccountRequestor: ThreadedChannelRequestor + Send + Sync {
    ...
    /// Takes an arbitrary response and a storage request, checks if the response matches. If it
    /// doesn't, it handles the out-of-order response and looks for the next response
    fn match_basic(
        &mut self,
        resp: DatabaseResponse<MAX_PREFETCH>,
        request: DatabaseRequest<MAX_PREFETCH>,
                ...
                match request {
                    DatabaseRequest::Basic(addr) => {
                        if addr == db_addr {
                            ...
                            // it doesnt match
                            self.cache_response(DatabaseResponse::Basic(
                                db_addr,
                                maybe_db_acct_info,
                            ));
                            self.get_and_handle_response(request, &Self::match_basic)
...
/// Function needed for executor.
pub trait ThreadedChannelStateRequestor:
    ThreadedChannelRequestor + ThreadedChannelAccountRequestor + Send + Sync
{
                ...
                match request {
                    DatabaseRequest::Storage(addr, slot) => {
                        if addr == db_addr && key == slot {
                            ...
                            // it doesnt match
                            self.cache_response(DatabaseResponse::Storage(
                                db_addr,
                                (key, maybe_value),
                            ));
                            self.get_and_handle_response(request, &Self::match_storage)
    ...
    /// Takes an arbitrary response and a bytecode by hash request, checks if the response matches.
    /// If it doesn't, it handles the out-of-order response and looks for the next response
    fn match_bytecode(
        &mut self,
        resp: DatabaseResponse<MAX_PREFETCH>,
        request: DatabaseRequest<MAX_PREFETCH>,
                ...
                match request {
                    DatabaseRequest::BytecodeFromHash(hash) => {
                        if hash == db_hash {
                            ...
                            // it doesnt match
                            self.cache_response(DatabaseResponse::BytecodeFromHash(
                                db_hash,
                                maybe_bytecode,
                            ));
                            self.get_and_handle_response(request, &Self::match_bytecode)
    ...
    /// Takes an arbitrary response and a blockhash request, checks if the response matches. If it
    /// doesn't, it handles the out-of-order response and looks for the next response
    fn match_block_hash(
        &mut self,
        resp: DatabaseResponse<MAX_PREFETCH>,
...
/// Account provider
pub trait ThreadedChannelAccountProvider:
    ThreadedChannelAccountRequestor + CacheAccountProvider + Send + Sync
{
    /// Get basic account information.
    ///
...
/// Function needed for executor.
pub trait ThreadedChannelStateProvider:
    ThreadedChannelStateRequestor + CacheStateProvider + Send + Sync
{
    /// Get storage.
    fn storage(
        &mut self,
