...
use crate::Result;
use crate::provider::{Error as ProviderError, AccountProvider, StateProvider, HeaderProvider, StateProviderFactory};
use crate::provider::threaded_db_requestor::*;
                    ...
                    DatabaseRequest::Basic(addr) => {
                        ...
                        match basic {
                            Ok(maybe_db_acct_info) => {
                                response_sender
                                    .send(
                                        DatabaseResponse::Basic(addr, maybe_db_acct_info)
                                    )
                                    .map_err(|_| ProviderError::BadChannelSend)?
                            }
                            Err(_e) => {
                                // TODO(brockelmore): send the error in a DatabaseResponse::Error wrapper
                            }
                        }
                    },
                    DatabaseRequest::Storage(addr, slot) => {
                        ...
                        match stor {
                            Ok(maybe_value) => {
                                response_sender
                                    .send(
                                        DatabaseResponse::Storage(addr, (slot, maybe_value))
                                    )
                                    .map_err(|_| ProviderError::BadChannelSend)?
                            }
                            Err(_e) => {
                    ...
                    DatabaseRequest::BlockHash(number) => {
                        ...
                        match hash {
                            Ok(maybe_hash) => {
                                response_sender
                                    .send(
                                        DatabaseResponse::BlockHash(number, maybe_hash)
                                    )
                                    .map_err(|_| ProviderError::BadChannelSend)?
                            }
                            Err(_e) => {
                    ...
                    DatabaseRequest::BytecodeFromHash(code_hash) => {
                        ...
                        match code {
                            Ok(maybe_code) => {
                                response_sender
                                    .send(
                                        DatabaseResponse::BytecodeFromHash(code_hash, maybe_code)
                                    )
                                    .map_err(|_| ProviderError::BadChannelSend)?
                            }
                            Err(_e) => {
